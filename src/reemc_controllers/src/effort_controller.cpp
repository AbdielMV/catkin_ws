...
namespace reemc_controllers{

class EffortController : public controller_interface::ControllerBase, public boost::noncopyable
{
public:
    ...
    //Mappings
    whole_body_state_msgs::WholeBodyState input_efforts;
    ...
    bool init(hardware_interface::EffortJointInterface* eff_iface, ros::NodeHandle &controller_nh){
        ...
        //Subsribe to joint references
        this->sub = controller_nh.subscribe("/reemc/efforts", 1 , &EffortController::EffortCallback, this);
    ...
    void writeJoints(whole_body_state_msgs::WholeBodyState input)
    {
        for(int k = 0; k < input.joints.size(); k++)
        {
            std::cout << input.joints[k].name << ": " << input.joints[k].effort << std::endl;
            for(unsigned int j = 0; j < joints_.size(); ++j)
            {
                //Check for common joints
                if(input.joints[k].name == joints_[j].getName())
                {
                    for(unsigned int i=0; i < controllerJointNames_.size(); i++)
                    {    
                        if(controllerJointNames_[i] == joints_[j].getName())
                        {
                            joints_[j].setCommand(input.joints[k].effort);
                            break;
                        }
                    }
    ...
    void EffortCallback(const whole_body_state_msgs::WholeBodyState &msg)
    {
        //Write joints
        std::cout << "working handsome : ) !!!" << std::endl;
        writeJoints(msg);
    }
...
PLUGINLIB_EXPORT_CLASS(reemc_controllers::EffortController, controller_interface::ControllerBase);
