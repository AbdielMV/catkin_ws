...
public:
  ...
  void someFunction(Rhonn& rhonn_object) {
    ...
    //Quitar despues todas estas impresiones de los valores que eran solamente debugg
    /*std::cout << "Value of h-> " << h << std::endl;
    std::cout << "Value of h.transpose-> " << h.transpose() << std::endl;
    std::cout << "Value of p_now-> " << p_now << std::endl;
...
int main(int argc, char** argv) {
  ...
  while (ros::ok()) {
    ...
    efk_object.error_estimation(neuron_1_value);
    efk_object.someFunction(neuron_1);
