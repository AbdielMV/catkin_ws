...
from curves import exact_cubic, curve_constraints, Quaternion, SO3Linear, SE3Curve
...
d_right=[0,0,0,0,0,0]
d_left=[0,0,0,0,0,0]
Kpv_matrix=-35*np.identity(3)
Kpw_matrix=-35*np.identity(3)
Kd_matrix=Kd=-2*np.sqrt(35)*np.identity(6)


##################################################

pub_multi_refence_right = rospy.Publisher('multi_reference_right', Accel, queue_size=10)
pub_multi_refence_left = rospy.Publisher('multi_reference_left', Accel, queue_size=10)
...
def robot_states(msg):
    ...
    for contact in msg.contacts:
        ...
        velocity = np.array([vel.linear.x, 
                             ...
                             vel.angular.y,
                             vel.angular.z])
        print("IMPRIMO VELOCIDAD")
        print(velocity)

        quaternion_ref = pinocchio.Quaternion(1,0.0,0.0,0.0)
        quaternion_ref_matrix = quaternion_ref.matrix()
        quaternion_matrix = quaternion.matrix()
        ...
        if((name=="right_sole_link") and (joint=="Right")):
            ...
            Pkkr_right_v=[pose.position.x-x_posicion,
                        pose.position.y-y_posicion,
                        pose.position.z-z_posicion
                        ]

            Pkkr_right_w = pinocchio.log3(quaternion_matrix.dot(quaternion_ref_matrix.transpose()))
        ...
        if((name=="left_sole_link") and (joint=="Left")):
            ...
            Pkkr_left_v=[pose.position.x-x_posicion,
                       pose.position.y-y_posicion,
                       pose.position.z-z_posicion
                       ]

            Pkkr_left_w = pinocchio.log3(quaternion_matrix.dot(quaternion_ref_matrix.transpose()))
    ...
    Pkkr_right_v = np.array(Pkkr_right_v)
    Pkkr_left_v  = np.array(Pkkr_left_v)
    Pkkr_right_v = Kpv_matrix.dot(Pkkr_right_v)
    Pkkr_left_v  = Kpv_matrix.dot(Pkkr_left_v)
    ...
    d_right[0] = Pkkr_right_v[0] + Tkkkr_right[0] + Tkkkr_dot_right[0] 
    d_right[1] = Pkkr_right_v[1] + Tkkkr_right[1] + Tkkkr_dot_right[1]
    d_right[2] = Pkkr_right_v[2] + Tkkkr_right[2] + Tkkkr_dot_right[2]
    d_right[3] = Pkkr_right_w[0] + Tkkkr_right[3] + Tkkkr_dot_right[3]
    d_right[4] = Pkkr_right_w[1] + Tkkkr_right[4] + Tkkkr_dot_right[4]
    d_right[5] = Pkkr_right_w[2] + Tkkkr_right[5] + Tkkkr_dot_right[5]

    d_left[0] = Pkkr_left_v[0] + Tkkkr_left[0] + Tkkkr_dot_left[0]
    d_left[1] = Pkkr_left_v[1] + Tkkkr_left[1] + Tkkkr_dot_left[1]
    d_left[2] = Pkkr_left_v[2] + Tkkkr_left[2] + Tkkkr_dot_left[2]
    d_left[3] = Pkkr_left_w[0] + Tkkkr_left[3] + Tkkkr_dot_left[3]
    d_left[4] = Pkkr_left_w[1] + Tkkkr_left[4] + Tkkkr_dot_left[4]
    d_left[5] = Pkkr_left_w[2] + Tkkkr_left[5] + Tkkkr_dot_left[5]
    ...
    p_multi_refence_right = Accel()
    p_multi_refence_left = Accel()

    p_multi_refence_right.linear.x = d_right[0]
    p_multi_refence_right.linear.y = d_right[1]
    p_multi_refence_right.linear.z = d_right[2]
    p_multi_refence_right.angular.x = d_right[3]
    p_multi_refence_right.angular.y = d_right[4]
    p_multi_refence_right.angular.z = d_right[5]

    p_multi_refence_left.linear.x = d_left[0]
    p_multi_refence_left.linear.y = d_left[1]
    p_multi_refence_left.linear.z = d_left[2]
    p_multi_refence_left.angular.x = d_left[3]
    p_multi_refence_left.angular.y = d_left[4]
    p_multi_refence_left.angular.z = d_left[5]
...
for j in range(10):
    ...
    init_quat = pinocchio.Quaternion(1,0.0,0.0,0.0)
    #init_quat = pinocchio.Quaternion(0.38,0.0,0.0,0.92)
    end_quat = pinocchio.Quaternion(1,0.0,0.0,0.0)
    init_rot = init_quat.matrix()
    end_rot = end_quat.matrix()
    ec2_rot[j] = SO3Linear(init_rot, end_rot, T0, T2)
    se3 = SE3Curve(ec2[j], ec2_rot[j])
    print("INTENTO IMPRIMIR ALGO")
