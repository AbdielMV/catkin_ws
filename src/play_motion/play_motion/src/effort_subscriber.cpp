...
namespace effort_subscriber{

class SubsEffortController : public controller_interface::ControllerBase, public boost::noncopyable
{
public:
    ...
    bool init(hardware_interface::EffortJointInterface* eff_iface, ros::NodeHandle &controller_nh){
        ...
        //Init callbacks
        ros::Subscriber effort_sub = controller_nh.subscribe("/reemc/status", 1, EffortCallback); 
            ...
            for(unsigned int j = 0; j<joints_.size(); ++j){
                ...
                for(unsigned int i=0; (i < controllerJointNames_.size()) && !found; ++i){
                    if(controllerJointNames_[i] == joints_[j].getName()){
                        ...
                        //std::cout << Tau_cmd_(i)*torqueConversions_(j) << std::endl;

                        //joints_[j].setCommand(Tau_cmd_(i)*torqueConversions_(j));
                        joints_[j].setCommand(torqueConversions_(j));
                        found = true;
                    }
                }
                if(!found){
    ...
    static void EffortCallback(const std_msgs::Bool)
    {
...
PLUGINLIB_EXPORT_CLASS(effort_subscriber::SubsEffortController, controller_interface::ControllerBase);
